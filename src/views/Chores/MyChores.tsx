...
export class MyChores extends React.Component {
  private FILTERS = {
    All: function (chores) {
      return chores
    },
      ...
      <Container maxWidth='md'>
        ...
        >
          ...
          <IconButton
            variant='outlined'
            color={
              selectedFilter && this.FILTERS[selectedFilter] && selectedFilter != 'All'
                ? 'primary'
                : 'neutral'
            }
            size='sm'
          ...
          >
            <Menu ref={menuRef}>
              {Object.keys(this.FILTERS).map((filter, index) => (
                <MenuItem
                  key={`filter-list-${filter}-${index}`}
                  onClick={() => {
                    const filterFunction = this.FILTERS[filter]
                ...
                >
                  ...
                  <Chip color={selectedFilter === filter ? 'primary' : 'neutral'}>
                    {this.FILTERS[filter].length === 2
                      ? this.FILTERS[filter](chores, userProfile.id).length
                      : this.FILTERS[filter](chores).length}
                  </Chip>
                </MenuItem>
              ))}
